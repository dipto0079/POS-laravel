...
//var shift_id = $('base[data-shift]').attr('data-shift');
...
$(document).ready(function () {
    ...
    $('#shift').on('change', function () {
        var switch_shift = $('#shift').val();
        //shift_id = shift;
        //$('base[data-shift]').attr('data-shift',shift);

        //alert(shift);
        show_all_items(switch_shift);
    });
    ...
    $('#walk_in_customer').on('change', function () {
        ...
        var disc = 0;
        if ($('option:selected',this).data("disc")) {
            disc = $('option:selected',this).data("disc");
        }

        //$("#others_discount").html(disc+'%');
        $("#others_discount").html(disc);
    ...
    $('#submit_table_modal').on('click', function () {
        ...
        //for auto select waiter based on table
        // var associate_waiter_id = $(this).data('waiter_id');

        // if (associate_waiter_id == "" || associate_waiter_id == 0 || associate_waiter_id == null) {
        // 	$("#select_waiter").val('').trigger("change");
        // } else {
        // 	$("#select_waiter").val(associate_waiter_id).trigger("change");
        // 	$("#select_waiter").attr("disabled", true);
        // }
    ...
    function bottom_add_click() {
        $('.bottom_add').on('click', function () {


            $('.bottom_add').attr("disabled", true);
            ...
            $('#submit_table_modal').data('waiter_id', waiter_id);
        });
        // return;
    }
    ...
    $('#table_modal_cancel_button,#proceed_without_table_button').on('click', function () {
        ...
        $("#select_floor").val('').trigger('change');
    ...
    $(document).on('click', '.remove_new_order_row_icon', function () {
        ...
        if (total_person <= 0) {
            ...
            $('.bottom_add').removeAttr("disabled");
    ...
    $('#notification_remove_all').on('click', function () {
        ...
        if ($('.single_notification_checkbox:checked').length > 0) {
            ...
            if (notifications != "") {
                ...
                $.ajax({
                    url: base_url + "restaurant/sale/remove_multiple_notification_ajax",
                    method: "POST",
                    data: {
    ...
    $(document).on('click', '.single_serve_b', function () {
        ...
        $.ajax({
            url: base_url + "restaurant/sale/remove_notication_ajax",
            method: "POST",
            data: {
            ...
            success: function (response) {
                $('#single_notification_row_' + response).remove();
                var total_notification = parseInt($('#notification_counter').html());
                $('#notification_counter').html(total_notification -1);
    ...
    $(document).on('mouseover', '.single_online_order, .single_hold_sale,.single_last_ten_sale', function () {
        $(this).css('background-color', '#f7f7f7');
    });
    $(document).on('mouseout', '.single_online_order, .single_hold_sale, .single_last_ten_sale', function () {
    ...
    $(document).on('click', '.single_hold_sale', function () {
        ...
        $.ajax({
            url: base_url + "restaurant/sale/get_single_hold_info_by_ajax",
            method: "POST",
            data: {
                hold_id: hold_id,
            },
            ...
            success: function (response) {
                ...
                if (response.waiter) {
                    $('#hold_waiter_name').html(response.waiter.manager_name);
                }else{
                ...
                if (response.customer) {
                    $('#hold_customer_name').html(response.customer.name);
                }else{
                ...
                if (response.table) {
                    $('#hold_table_name').html(response.table.name);
                }else{
                ...
                for (var key in response.items) {
                    ...
                    var i = 1;
                    // console.log(this_item.modifiers);
    ...
    $('#delete_all_hold_sales_button').on('click', function () {
        if ($('.detail_hold_sale_holder .single_hold_sale').length > 0) {
            ...
            if (r == true) {
                ...
                $.ajax({
                    url: base_url + "restaurant/sale/delete_all_holds_with_information_by_ajax",
                    method: "POST",
                    data: {
                    },
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    ...
    $('#hold_delete_button').on('click', function () {
        if ($('.single_hold_sale[data-selected=selected]').length > 0) {
            ...
            if (r == true) {
                //delete all information of hold based on hold_id
                $.ajax({
                    url: base_url + "restaurant/sale/delete_all_information_of_hold_by_ajax",
                    method: "POST",
                    data: {
                        hold_id: hold_id,
                    },
    ...
    $('#online_order_close_button,#online_order_close_button_cross').on('click', function () {
        $('#online_order_waiter_id').html('');
        $('#online_order_waiter_name').html('');
        $('#online_order_customer_id').html('');
        $('#online_order_customer_name').html('');
        $('#online_order_table_id').html('');
        $('#online_order_table_name').html('');
        $('#online_order_order_type').html('');
        $('#online_order_order_type_id').html('');

        $(".item_modifier_details .modifier_item_details_holder").empty();
        $("#total_items_in_cart_online_order").html('0');
        $("#sub_total_show_online_order").html('0.00');
        $("#sub_total_online_order").html('0.00');
        $("#total_item_discount_online_order").html('0.00');
        $("#discounted_sub_total_amount_online_order").html('0.00');
        $("#sub_total_discount_online_order").html('');
        $("#sub_total_discount_amount_online_order").html('0.00');
        $("#all_items_vat_online_order").html('0.00');
        $("#all_items_discount_online_order").html('0.00');
        $("#delivery_charge_online_order").html('0.00');
        $("#total_payable_online_order").html('0.00');
        $('#show_online_order_modal').slideUp(333);

    });

    //online order show
    $('#open_online_order').on('click', function () {
        $('#show_online_order_modal').slideDown(333);
        get_all_online_orders();
    });

    //show single online order in modal



    $(document).on('click', '.single_online_order', function () {
        //get Online order id
        var online_order_id = $(this).attr('id').substr(13);
        $('.single_online_order').css('background-color', '#ffffff');
        $('.single_online_order').attr('data-selected', 'unselected');
        $(this).css('background-color', '#f7f7f7');
        $(this).attr('data-selected', 'selected');
        //get all info of online order based on online_order_id

        $.ajax({
            url: base_url + "restaurant/sale/get_single_online_order_info_by_ajax",
            method: "POST",
            data: {
            ...
            success: function (response) {
                ...
                var order_type = '';
                var order_type_id = 0;
                $('#online_order_waiter_id').html(response.waiter_id);
                if (response.waiter) {
                    $('#online_order_waiter_name').html(response.waiter.manager_name);
                }else{
                    $('#online_order_waiter_name').html('');
                }

                $('#online_order_customer_id').html(response.customer_id);

                if (response.customer) {
                    $('#online_order_customer_name').html(response.customer.name);
                }else{
                    $('#online_order_customer_name').html('');
                }

                $('#online_order_table_id').html(response.table_id);
                if (response.table) {
                    $('#online_order_table_name').html(response.table.name);
                }else{
                    $('#online_order_table_name').html('');
                }
                if (response.order_type == 1) {
                ...
                $('#online_order_order_type').html(order_type);
                $('#online_order_order_type_id').html(order_type_id);
                var draw_table_for_online_order = '';
                ...
                for (var key in response.items) {
                    ...
                    // console.log(selected_modifiers);
                    var discount_value = (this_item.menu_discount_value != "") ? this_item.menu_discount_value : '0.00';
                    draw_table_for_online_order += '<div class="single_item_modifier fix" id="online_order_for_item_' + this_item.food_menu_id + '">';
                    draw_table_for_online_order += '<div class="first_portion fix">';
                    draw_table_for_online_order += '<span class="item_vat_online_order" id="online_order_item_vat_percentage_table' + this_item.food_menu_id + '" style="display:none;">' + this_item.menu_vat_percentage + '</span>';
                    draw_table_for_online_order += '<span class="item_discount_online_order" id="online_order_item_discount_table' + this_item.food_menu_id + '" style="display:none;">' + this_item.discount_amount + '</span>';
                    draw_table_for_online_order += '<span class="item_price_without_discount_online_order" id="online_order_item_price_without_discount_' + this_item.food_menu_id + '" style="display:none;">' + this_item.menu_price_without_discount + '</span>';
                    draw_table_for_online_order += '<div class="single_order_column_hold first_column column fix"><span id="online_order_item_name_table_' + this_item.food_menu_id + '">' + this_item.menu_name + '</span></div>';
                    draw_table_for_online_order += '<div class="single_order_column_online_order second_column column fix">' + currency + ' <span id="online_order_item_price_table_' + this_item.food_menu_id + '">' + this_item.menu_unit_price + '</span></div>';
                    draw_table_for_online_order += '<div class="single_order_column_online_order third_column column fix"><span id="online_order_item_quantity_table_' + this_item.food_menu_id + '">' + this_item.qty + '</span></div>';
                    draw_table_for_online_order += '<div class="single_order_column_online_order forth_column column fix"><span class="online_order_special_textbox" id="online_order_percentage_table_' + this_item.food_menu_id + '">' + discount_value + '</span></div>';
                    draw_table_for_online_order += '<div class="single_order_column_online_order fifth_column column fix">' + currency + ' <span id="online_order_item_total_price_table_' + this_item.food_menu_id + '">' + this_item.menu_price_with_discount + '</span></div>';
                    draw_table_for_online_order += '</div>';
                    if (selected_modifiers != "") {
                        draw_table_for_online_order += '<div class="second_portion fix">';
                        draw_table_for_online_order += '<span id="online_order_item_modifiers_id_table_' + this_item.food_menu_id + '" style="display:none;">' + selected_modifiers_id + '</span>';
                        draw_table_for_online_order += '<span id="online_order_item_modifiers_price_table_' + this_item.food_menu_id + '" style="display:none;">' + selected_modifiers_price + '</span>';
                        draw_table_for_online_order += '<div class="single_order_column_online_order first_column column fix"><span id="online_order_item_modifiers_table_' + this_item.food_menu_id + '">' + selected_modifiers + '</span></div>';
                        draw_table_for_online_order += '<div class="single_order_column_online_order second_column column fix">' + currency + ' <span id="online_order_item_modifiers_price_table_' + this_item.food_menu_id + '">' + modifiers_price + '</span></div>';
                        draw_table_for_online_order += '</div>';
                    }
                    if (this_item.menu_note != "" && this_item.menu_note != null) {
                        draw_table_for_online_order += '<div class="third_portion fix">';
                        draw_table_for_online_order += '<div class="single_order_column_online_order first_column column fix">Note: <span id="online_order_item_note_table_' + this_item.food_menu_id + '">' + this_item.menu_note + '</span></div>';
                        draw_table_for_online_order += '</div>';
                    }
                ...
                //add to top
                $(".item_modifier_details .modifier_item_details_holder").empty();
                $(".item_modifier_details .modifier_item_details_holder").prepend(draw_table_for_online_order);

                var total_items_in_cart_with_quantity = 0;
                $('.detail_online_order_holder .modifier_item_details_holder .single_item_modifier .first_portion .third_column span').each(function (i, obj) {
                    total_items_in_cart_with_quantity = parseInt(total_items_in_cart_with_quantity) + parseInt($(this).html());
                });
                $('#total_items_in_cart_online_order').html(total_items_in_cart_with_quantity);
                var sub_total_discount_hold = (response.sub_total_discount_value != "") ? response.sub_total_discount_value : "0.00";
                $("#sub_total_show_online_order").html(response.sub_total);
                $("#sub_total_online_order").html(response.sub_total);
                $("#total_item_discount_online_order").html(response.total_item_discount_amount);
                $("#discounted_sub_total_amount_online_order").html(response.sub_total_discount_amount);
                $("#sub_total_discount_online_order").html(sub_total_discount_hold);
                $("#sub_total_discount_amount_online_order").html(response.sub_total_with_discount);
                var total_vat_section_to_show = '';
                $.each(JSON.parse(response.sale_vat_objects), function (key, value) {
                    total_vat_section_to_show += '<span class="tax_field_order_details" id="tax_field_order_details_' + value.tax_field_id + '">' + value.tax_field_type + '</span>: ' + currency + ' <span class="tax_field_amount_order_details" id="tax_field_amount_order_details_' + value.tax_field_id + '">' + value.tax_field_amount + '</span><br/>';
                });
                $("#all_items_vat_online_order").html(total_vat_section_to_show);
                $("#all_items_discount_online_order").html(response.total_discount_amount);
                $("#delivery_charge_online_order").html(response.delivery_charge);
                $("#total_payable_online_order").html(response.total_payable);
                // $(".order_holder_hold").prepend('<span id="modification_'+sale_id+'" class="modification" style="display:none;">'+sale_id+'</span>');
                //do calculation on table
            },
            error: function () {
    ...
    //accepte online order and send to running order
    $('#online_order_accepte_to_running_order_button').on('click', function () {

        if ($('.single_online_order[data-selected=selected]').length > 0) {
            var online_order_id = $('.single_online_order[data-selected=selected]').attr('id').substr(13);
            ...
            $.ajax({
                url: base_url + "restaurant/sale/add_online_order_to_sale_by_ajax",
                method: "POST",
                data: {
                    //order: order_object,
                    online_order_id: online_order_id,
                ...
                success: function (response) {
                    ...
                    //
                    //     draw_table_for_order += '<div class="single_order_column first_column fix"><span style="cursor:pointer;" class="edit_item" id="edit_item_' + this_item.food_menu_id + '"> <span id="item_name_table_' + this_item.food_menu_id + '">' + this_item.menu_name + '</span> </span></div>';
                    //
                    //
            ...
            swal({
                title: warning + "!",
                text: 'Please select an online order first',
                confirmButtonText: ok,
                confirmButtonColor: '#b6d6f6'
            })
        }
    ...
    $('#create_invoice_and_close,#order_details_create_invoice_close_order_button').on('click', function () {
        ...
        if ($('.holder .order_details > .single_order[data-selected=selected]').length > 0) {
            ...
            var sale_id = $('.holder .order_details .single_order[data-selected=selected]').attr('id').substr(6);
            $.ajax({
                url: base_url + "restaurant/sale/get_all_information_of_a_sale_ajax",
                method: "POST",
                data: {
                    sale_id: sale_id,
                },
    ...
    $('#given_amount_input').on('keyup', function (e) {
        ...
        var tips = ($('#tips_amount_input').val() != "") ? $('#tips_amount_input').val() : 0;
        var total_change = (parseFloat(given_amount) - parseFloat(total_payable) - parseFloat(tips)).toFixed(2);
    ...
    $('#tips_amount_input').on('keyup', function (e) {
        //if the letter is not digit then display error and don't type anything
        if (e.which != 110 && !(e.which < 106 && e.which > 95) && e.which != 190 && e.which != 16 && e.which != 53 && e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
            $(this).val('');
        }
        var tips_amount = ($(this).val() != "") ? $(this).val() : 0;

        remove_last_two_digit_without_percentage(tips_amount, $(this));
        //calculate_create_invoice_modal();
    });
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (order_type == 1) {
                        ...
                        order_number = 'T ' + order_object.sale_no;
                        order_type_name = 'Take Away';
                    } else if (order_type == 3) {
                        order_number = 'DL ' + order_object.sale_no;
                        order_type_name = 'Delivery';
                    }
                    var tables_booked = '';

                    if(order_object.table != null){
                        tables_booked = order_object.table.name;
                    }else{
                        tables_booked = 'None';
                    }

                    var waiter_name = (order_object.waiter_id == "" || order_object.waiter_id == null) ? "" : order_object.waiter.manager_name;
                    var customer_name = (order_object.customer_id == "" || order_object.customer_id == null) ? "" : order_object.customer.name;
                    var table_name = (order_object.table_id == "" || order_object.table_id == null) ? "" : tables_booked;
                    // var order_time = new Date(Date.parse(order_object.sale_date));
                    var order_time = new Date(Date.parse(order_object.processing_date_time));
                    var now = new Date();
                    ...
                    var order_placed_at = 'Order Placed at: ' + order_time.getDate() + '-'+ order_time.getMonth() + ' '+ order_time.getHours() + ':' + order_time.getMinutes() + '';
                    var item_list_to_show = '';
                    for (var key in order_object.sales_details) {
                        var single_item = order_object.sales_details[key];
                        ...
                        if (single_item.cooking_status == 'Started Cooking') {
                            ...
                            // var cooking_start_time = new Date(Date.parse(single_item.cooking_start_time));
                            var cooking_start_time = new Date(Date.parse(single_item.cooking_done_time));
                            var now = new Date();
    ...
    $("#search").on('keyup', function (e) {
        ...
        for (var key in foundItems) {
            ...
            var shift_check = false;
            //check that the food menu is for the current shift
            foundItems[key].shifts.forEach(shift => {
                //console.log(shift.menu_shift_id);
                //return false;
                if (shift.menu_shift_id == shift_id) {
                    shift_check = true;
                }
            });

            if (foundItems.hasOwnProperty(key) && shift_check == true) {
                searched_category_items_to_show += '<div class="single_item fix" id="item_' + foundItems[key].item_id + '">';
                searched_category_items_to_show += '<p class="item_price">'+ foundItems[key].price +' | ' + foundItems[key].t_price +' | ' + foundItems[key].dl_price + '</p>';
    ...
    $('#dine_in_button,#take_away_button,#delivery_button').on('click', function () {
        ...
        if ($(this).attr('id') == 'dine_in_button') {} else if ($(this).attr('id') == 'take_away_button') {
            ...
            $('#select_waiter').removeAttr('disabled');
            //$("#select_waiter").val('').trigger("change");
        ...
        if (total_items_in_cart > 0) {
            var r = confirm("Cart is not empty, want to proceed?shuvo");
            if (r == true) {
                $('.order_holder').empty();
            }
        }
    ...
    //add foodmenus to order table
    //when single item is clicked pop-up modal is appeared
    $(document).on('click', '.single_item', function () {
        ...
        if (selected_order_type_object.length > 0) {
            ...
            if (selected_order_type_object.attr('id') == 'delivery_button') {
                ...
                item_price = (parseFloat($('#dl_price_' + item_id).html())).toFixed(2);

            } else if (selected_order_type_object.attr('id') == 'dine_in_button') {
                order_type = 1;
                //get item/menu price from modal
                item_price = (parseFloat($('#d_price_' + item_id).html())).toFixed(2);

            } else if (selected_order_type_object.attr('id') == 'take_away_button') {
                order_type = 2;
                //get item/menu price from modal
                item_price = (parseFloat($('#t_price_' + item_id).html())).toFixed(2);
            ...
            swal({
                title: warning + "!",
                text: 'select Dine In or Take-away or delivery',
                confirmButtonText: ok,
                confirmButtonColor: '#b6d6f6'
            })
            return false;
        ...
        // alert(item_total_price_without_discount);
        ...
        if (tax_information.length > 0) {
            for (const  k in tax_information) {
        ...
        draw_table_for_order += '<div class="single_order_column first_column fix"><span style="cursor:pointer;" class="edit_item" id="edit_item_' + item_id + '"> <span id="item_name_table_' + item_id + '">' + item_name + '</span> </span> </div>';
    ...
    $('#close_set_processing_date_time').on('click', function () {
        $('#processing_date').css('border', '1px solid #B5D6F6');
        $('#processing_time').css('border', '1px solid #B5D6F6');
        reset_on_modal_close_or_set_pocessing_date_time();
    });
    ...
    //this function is called and show foodmenus on basis of shift status change
    function call_for_shift_food_menus(){

        $.ajax({
            url: base_url + "restaurant/sale/food-menus-change-for-shift-ajax",
            method: "GET",
            data:{
                shift_id : shift_id
            },
            success: function (data) {
                ...
                if (data.shift_change == true) {

                    $('base[data-shift]').attr('data-shift', data.shift.id);
                    //need to change here
                    //window.items ='';
                    console.log(['changed', data.shift.id, role]);
                    if (role !== 'Manager') {
                        location.reload();
                    }else{
                        ...
                        // console.log(data.food_menus [0].shift_id);
                        //var food_menus = data.food_menus;
                        // console.log(data.food_menus);

                        var food_menus = searchItemAndConstructGallery('');
                        console.log(food_menus);

                        $(".specific_category_items_holder").remove('');


                        var category_items_to_show = '';
                        for (var i in food_menus ) {

                            var shift_check = false;
                            //check that the food menu is for the current shift
                            food_menus[i].shifts.forEach(shift => {
                                //console.log(shift.menu_shift_id);
                                //return false;
                                if (shift.menu_shift_id == shift_id) {
                                    shift_check = true;
                            ...
                            if (food_menus.hasOwnProperty(i) && shift_check == true) {
                                console.log(food_menus[i].price);
                                // var image_path ='';
                                // if(food_menus [i].image != null){
                                // 	image_path = base_url+'media/restaurant/photos/'+food_menus [i].image;
                                // }else{
                                // 	image_path = base_url+'assets/images/image_thumb.png';
                                // }

                                category_items_to_show = '<div id="category_'+ food_menus[i].category_id + '"class="specific_category_items_holder" style="display:flex;">';
                                category_items_to_show += '<div class="single_item fix" id="item_' + food_menus[i].id + '">';
                                category_items_to_show += '<p class="item_price">'+ food_menus[i].price +' | ' + food_menus[i].t_price +' | ' + food_menus[i].dl_price +'</p>';
                                category_items_to_show += '<img src="' + food_menus [i].image + '" alt="" width="141">';
                                category_items_to_show += '<p class="item_name">' + food_menus [i].item_name + ' (' + food_menus [i].item_code + ')</p>';
                                category_items_to_show += '</div>';
                                category_items_to_show += '<div>';
                            }
                        }
                        ...
                        $("#searched_item_found").remove();
                        $('.specific_category_items_holder').hide('1000');
                        $(".category_items").prepend(category_items_to_show);

                        var searched_category_items_to_show = '<div id="searched_item_found" class="specific_category_items_holder" style="display:flex;">';
                        for (var key in food_menus) {

                            var shift_check = false;
                            //check that the food menu is for the current shift
                            food_menus[key].shifts.forEach(shift => {
                                //console.log(shift.menu_shift_id);
                                //return false;
                                if (shift.menu_shift_id == shift_id) {
                                    shift_check = true;
                            ...
                            // var image_path ='';
                            // if(food_menus[key].image != null){
                            // 	image_path = base_url+'media/restaurant/photos/'+food_menus[key].image;
                            // }else{
                            // 	image_path = base_url+'assets/images/image_thumb.png';
                            // }
                            if (food_menus.hasOwnProperty(key) && shift_check== true) {
                                //console.log(food_menus[key]);
                                searched_category_items_to_show += '<div class="single_item fix" id="item_' + food_menus[key].item_id + '">';
                                searched_category_items_to_show += '<p class="item_price">shuvo'+ food_menus[key].price +' | ' + food_menus[key].t_price +' | ' + food_menus[key].dl_price +'</p>';
                                searched_category_items_to_show += '<span class="item_vat_percentage" style="display:none;">' + food_menus[key].vat_percentage + '</span>';
                                searched_category_items_to_show += '<img src="' + food_menus[key].image + '" alt="" width="141">';
                                searched_category_items_to_show += '<p class="item_name">' + food_menus[key].item_name + ' (' + food_menus[key].item_code + ')</p>';
                                searched_category_items_to_show += '</div>';
                            }
                        }
                        searched_category_items_to_show += '<div>';
                ...
                // if(data.no_food_menus){
                // 	$(".category_items").html(data.no_food_menus);
                // 	console.log(data.no_food_menus);
                // }
                // if(data.same_food_menus){
                // 	$(".category_items").html(data.same_food_menus);
                // 	console.log(data.same_food_menus);
                // }
    ...
    function shift_interval(){
        // ==================================================
        $('.specific_category_items_holder').hide();

        setInterval(function () {

            call_for_shift_food_menus ();
    ...
    show_all_items();
    //initialize item list with shift wise interval
    shift_interval();
    ...
    $('#close_delivery_method').on('click', function () {
        reset_on_modal_close_or_set_delivery_method();
    });

    function reset_on_modal_close_or_set_delivery_method() {
        $('#delivery_method_modal').slideUp(333);
        $("#delivery_method_modal").find('input[name="delivery_method"]:checked').prop('checked',false);

    }

    // set processing date and time
    $('#set_delivery_method').on('click', function () {
        var delivery_method = $("#delivery_method_modal").find('input[name="delivery_method"]:checked').val();
        // alert(delivery_method);
        // return false;

        var error = 0;

        $('#delivery_method').css('border', '1px solid #B5D6F6');

        if (delivery_method == "") {
            $('#delivery_method').css('border', '1px solid red');
            error++;
        }
        ...
        // $('#processing_date_time').html(pros_date_time);
        // $('#hidden_processing_date_time').val(pros_date_time);
        $('#delivery_method_modal').slideUp(333);
    ...
    $('#delivery_button').on('click', function () {
        ...
        // }
        $('#delivery_method_modal').slideDown(333);
    ...
    $('#table_button,#dine_in_button').on('click', function () {
        ...
        $('#select_floor').on('change', function () {
            if ($(this).val() !== '') {
                $.ajax({
                    url: base_url + "restaurant/sale/get_floor_with_tables_ajax",
                    method: "GET",
                    data:{
                        floor_id: $(this).val()
                    },
                    success: function (data) {
                        ...
                        // 		$tables_modal += '<div class="floatleft fix single_order_table" id="single_table_info_holder_'+table_details[key].id+'">';
                        // 		$tables_modal += '<p class="table_name" style="font-weight:bold;"><span id="sit_name_'+table_details[key].id+'">'+table_details[key].name+'<span></p>';
                        // 		$tables_modal += '<p class="table_sit_capacity">Sit Capacity: <span id="sit_capacity_number_'+table_details[key].id+'">'+table_details[key].guest_count+'<span></p>';
                        // 		$tables_modal += '<p class="table_available">Available: <span id="sit_available_number_'+table_details[key].id+'">'+table_details[key].guest_count+'</span></p>';
                        ...
                        // 		$tables_modal += '<div class="single_table_order_details_holder fix" id="single_table_order_details_holder_'+table_details[key].id+'">';
                        // 			$tables_modal += '<div class="top fix" id="single_table_order_details_top_'+table_details[key].id+'">';
                        ...
                        // 				// }
                        // 				if ($('#single_table_order_details_top_' + table_details[key].id + ' .new_book_to_table').length > 0) {
                        // 					var new_row = $('#single_table_order_details_top_' + table_details[key].id + ' .new_book_to_table').html();
                        // 					tables_modal += '<div class="single_row fix new_book_to_table"  id="new_order_table_' + table_details[key].id + '">';
                        ...
                        // 			$tables_modal += '<div class="bottom fix" id="single_table_order_details_bottom_'+table_details[key].id+'">';
                        // 				$tables_modal += '<input type="text" name="" placeholder="Order" class="floatleft bottom_order"  id="single_table_order_details_bottom_order_'+table_details[key].id+'" readonly>';
                        // 				$tables_modal += '<input type="text" name="" placeholder="Person" class="floatleft bottom_person" id="single_table_order_details_bottom_person_'+table_details[key].id+'">';
                        // 				$tables_modal += '<button class="floatleft bottom_add" data-waiter_id="'+ table_details[key].waiter_id +'" id="single_table_order_details_bottom_add_'+table_details[key].id+'">Add</button>';
                        // 			$tables_modal += '</div>';
                        // 		$tables_modal += '</div>';
                        // 	$tables_modal += '</div>';
                        ...
                        // }
                        // $('#table').html('');
                        // $('#table').append($tables_modal);

                        // $('.bottom_person').val('1');

                        // // //for enable table add
                        // bottom_add_click();
    ...
    function current_local_date_time() {
        var date_time = new Date(new Date().toString().split('GMT')[0]+' UTC').toISOString().split('.')[0];
        return date_time;
    }

    // show processing date time modal
    function processing_date_time(){
        $('#processing_date_time_modal').slideDown(333);
        var current_date_time =  current_local_date_time();
        $('#processing_date_time_modal_input').val(current_date_time);
        $('#processing_date_time_modal_input').attr("min", current_date_time);

    }

    // set processing date and time
    $('#set_processing_date_time').on('click', function () {
        var pros_date_time = $('#processing_date_time_modal_input').val();

        var error = 0;

        $('#processing_date_time_modal_input').css('border', '1px solid #B5D6F6');

        if (pros_date_time == "") {
            $('#processing_date_time_modal_input').css('border', '1px solid red');
            error++;
        }
        ...
        $('#processing_date_time').html(pros_date_time);
        $('#hidden_processing_date_time').val(pros_date_time);
        $('#processing_date_time_modal').slideUp(333);
    ...
    $('#place_order_operation').on('click', function () {
        ...
        var pros_date_time = $('#hidden_processing_date_time').val();

        // console.log(pros_date_time);

        var selected_order_type_object = $('.main_top').find('button[data-selected=selected]');
        var total_items_in_cart = $('.order_holder .single_order').length;
        var sub_total = parseFloat($('#sub_total_show').html()).toFixed(2);
        var total_vat = parseFloat($('#all_items_vat').html()).toFixed(2);
        // console.log(parseFloat($('#all_items_vat').html()));
        // return false
        ...
        //add this for other discount cal
        var total_others_discount = parseFloat($('#total_others_discount').html()).toFixed(2);
        ...
        //shuvo
        var delivery_method_id = $("#delivery_method_modal").find('input[name="delivery_method"]:checked').val();

        // alert(delivery_method_id);
        // return false;
        ...
        if (selected_order_type_object.length > 0) {
            ...
            if (selected_order_type_object.attr('id') == 'delivery_button') {
                order_type = 3;
                //it's not confirm that it is been here or not
                if (waiter_id == "") {
                    /*swal({
						title: warning + "!",
                ...
                if(typeof delivery_method_id === "undefined" && sale_id==0){
                    swal({
                        title: "Alert!",
                ...
                // if(selected_table==0 && selected_order_type_object == 1){
                // 	swal({
                //               title: "Alert!",
                //               text: "Please select a Table!",
                //               confirmButtonColor: '#b6d6f6'
            ...
            if ( pros_date_time == '' && order_type != 1) {
                processing_date_time();
                return false;
            }

            if (typeof delivery_method_id == "undefined" && order_type !=  3) {
                delivery_method_id = "";
            }
            ...
            order_info += '"delivery_method_id":"' + delivery_method_id + '",';
            ...
            order_info += '"total_others_discount":"' + total_others_discount + '",';
            ...
            order_info += '"processing_date_time":"' + pros_date_time + '",';
            ...
            if (order_type == 1) {
                ...
                orders_table += '{"table_id":"' + selected_table +'"}';
            ...
            // console.log(JSON.parse(order_object));
            // return false;
    ...
    $('#finalize_order_button').on('click', function () {
        ...
        if ($('.holder .order_details > .single_order[data-selected=selected]').length > 0) {
            ...
            var tips = $('#tips_amount_input').val();
            ...
            if (payment_method_type == 3 || payment_method_type == 'gift card') {
                card_no = $('#payment_gift_card_no_modal_input').val();
                if (card_no == "") {
            ...
            print_invoice_and_close(sale_id, payment_method_type, invoice_create_type, paid_amount, due_amount, card_no, tips);
            reset_finalize_modal();
        } else {
            swal({
                title: warning + "!",
    ...
    $('#add_customer').on('click', function () {
        ...
        var customer_note = $('#customer_note_modal').val();
        var customer_city = $('#city').val();
        var customer_country = $('#country').val();
        var customer_state = $('#state').val();
        var customer_apt = $('#customer_apt_number_modal').val();
        var customer_zip = $('#customer_zip_modal').val();
        var customer_code = $('#customer_code_modal').val();
        var customer_group = $('#customer_group_modal').val();

        // console.log([customer_id, customer_name, customer_phone, customer_email, customer_delivery_address, customer_note, customer_city, customer_country, customer_state, customer_group]);
        // return false;
        ...
        $('#customer_name_modal').css('border', '1px solid #B5D6F6');
        $('#customer_phone_modal').css('border', '1px solid #B5D6F6');
        $('#customer_zip_modal').css('border', '1px solid #B5D6F6');
        $('#customer_code_modal').css('border', '1px solid #B5D6F6');
        $('#customer_note_modal').css('border', '1px solid #B5D6F6');
        ...
        if (customer_zip == "") {
            $('#customer_zip_modal').css('border', '1px solid red');
            error++;
        }

        if (customer_code == "") {
            $('#customer_code_modal').css('border', '1px solid red');
            error++;
        }
        if (customer_note == "") {
            $('#customer_note_modal').css('border', '1px solid red');
            error++;
        }
        ...
        $.ajax({
            url: base_url + "restaurant/sale/add_customer_by_ajax",
            method: "POST",
            data: {
            ...
            success: function (data) {
                ...
                // if (data > 0) {
                $.ajax({
                    url: base_url + "restaurant/sale/get_all_customers_for_this_user",
                    method: "GET",
                    success: function (response) {
                        // response = JSON.parse(response);
...
function _serarch_auto(str)
{
    // if (e.keyCode == 13) {
    var shift_id = $('#shift').val();
    var searched_string = str.trim();
    var foundItems = searchItemAndConstructGallery(searched_string);
    var searched_category_items_to_show = '<div id="searched_item_found" class="specific_category_items_holder" style="display:flex;">';
...
// ==================================================
function show_all_items(switch_shift = shift_id) {
    ...
    setTimeout(function () {
        ...
        for (var key in foundItems) {
            ...
            foundItems[key].shifts.forEach(shift => {
                ...
                // 	shift_check = true;
                // }
                if (shift.menu_shift_id == switch_shift) {
                    shift_check = true;
                }
            });
            //return false;
            if (foundItems.hasOwnProperty(key) && shift_check == true) {
                console.log(foundItems[key].price);
                searched_category_items_to_show += '<div class="single_item fix" id="item_' + foundItems[key].item_id + '">';
                searched_category_items_to_show += '<p class="item_price">'+ foundItems[key].price +' | ' + foundItems[key].t_price +' | ' + foundItems[key].dl_price + '</p>';
                searched_category_items_to_show += '<span class="item_vat_percentage" style="display:none;">' + foundItems[key].vat_percentage + '</span>';
                searched_category_items_to_show += '<img src="' + foundItems[key].image + '" alt="" width="141">';
    ...
    $('#hold_sale').on('click', function () {
        if ($('.order_holder .single_order').length > 0) {
            $.ajax({
                url: base_url + "restaurant/sale/get_new_hold_number_ajax",
                method: "GET",
                success: function (response) {
    ...
    $('#hold_cart_info').on('click', function () {
        ...
        // console.log(JSON.parse(order_info));
...
function calculate_others_discount(get_sub_total_amount, get_others_discount_percentage) {
    var others_discount_percentage = (get_others_discount_percentage != "") ? get_others_discount_percentage : 0;
    var sub_total_amount = (get_sub_total_amount != "") ? get_sub_total_amount : 0;
    //return (((parseFloat(sub_total_amount) * parseFloat(others_discount_percentage)) / parseFloat(100))).toFixed(2);
    var total_others_discount = (((parseFloat(sub_total_amount) * parseFloat(others_discount_percentage)) / parseFloat(100))).toFixed(2);
    $('#total_others_discount').html(total_others_discount);
}
...
function do_addition_of_item_and_modifiers_price() {
    ...
    //others discount cal
    calculate_others_discount(sub_total_amount, $('#others_discount').html())
    ...
    //get total others discount amount
    var total_others_discount_amount = (parseFloat($('#total_others_discount').html())).toFixed(2);
    ...
    var total_discount = (parseFloat(sub_total_discount_amount) + parseFloat(total_item_discount_amount) + parseFloat(total_others_discount_amount)).toFixed(2);
    ...
    //change this for others discount cal
    var discounted_sub_total_amount = (parseFloat(sub_total_amount) - parseFloat(sub_total_discount_amount) - parseFloat(total_others_discount_amount)).toFixed(2);
...
function reset_on_modal_close_or_add_customer() {
    ...
    $('#customer_apt_number_modal').val('');
    $('#customer_zip_modal').val('');
    $('#customer_code_modal').val('');
    $('#customer_group_modal').val('').trigger('change');
    $('#customer_delivery_address_modal').val('');
    $('#customer_note_modal').val('');
}

function reset_on_modal_close_or_set_pocessing_date_time() {
    $('#processing_date_time_modal').slideUp(333);
    $('#processing_date').val('');
    $('#processing_time').val('');
}
...
function add_sale_by_ajax(order_object, sale_id) {
    ...
    $.ajax({
        url: base_url + "restaurant/sale/add_sale_by_ajax",
        method: "POST",
        data: {
            order: order_object,
            sale_id: sale_id,
...
function add_hold_by_ajax(order_object, hold_number) {
    $.ajax({
        url: base_url + "restaurant/sale/add_hold_by_ajax",
        method: "POST",
        data: {
            order: order_object,
...
function set_new_orders_to_view() {
    ...
    $.ajax({
        url: base_url + "restaurant/sale/get_new_orders_ajax",
        method: "GET",
        success: function (response) {
            response = JSON.parse(response);
            //console.log('after placed the order');
            //console.log(response);
            $order_list_left = '';
            var i = 1;
            for (var key in response) {
                ...
                if (i == 1) {
                    $order_list_left += '<div data-started-cooking="' + total_kitchen_type_started_cooking_items + '" data-done-cooking="' + total_kitchen_type_done_items + '" class="single_order fix" style="margin-top:0px" data-selected="unselected" id="order_' + response[key].sale_id + '">';
                } else {
                    $order_list_left += '<div data-started-cooking="' + total_kitchen_type_started_cooking_items + '" data-done-cooking="' + total_kitchen_type_done_items + '" class="single_order fix" data-selected="unselected" id="order_' + response[key].sale_id + '">';
                }
                ...
                if (response[key].order_type == '1') {
                    ...
                    order_name = 'T ' + response[key].sale_no;
                } else if (response[key].order_type == '3') {
                    order_name = 'DL ' + response[key].sale_no;
                }


                //var table_name = (response[key].tables_booked[0].table_name!=null)?response[key].tables_booked[0].table_name:"";
                var table_name = (response[key].table_id != null) ? response[key].table.name : "";
                //console.log(table_name);
                var waiter_name = (response[key].waiter.manager_name != null) ? response[key].waiter.manager_name : "";
                var customer_name = (response[key].customer.name != null) ? response[key].customer.name : "";
                ...
                var tables_booked = '';
                //if (response[key].tables_id.length > 0) {
                if (response[key].table_id != null) {
                    // var w = 1;
                ...
                $order_list_left += '</div>';
                $order_list_left += '<div class="order_condition">';
                $order_list_left += '<p style="font-size:16px;">Time Count: <span id="order_minute_count_' + response[key].sale_id + '">' + minute + '</span>:<span id="order_second_count_' + response[key].sale_id + '">' + second + '</span> M</p>';
                $order_list_left += '</div>';
                $order_list_left += '</div>';
                $order_list_left += '</div>';
                i++;
            ...
            reset_table_modal();
            createAnimation(response[key].sale_id);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var key in response) {
                ...
                if (i == 1) {
                    if (last_key == key) {
                        $order_list_left += '<div data-started-cooking="' + total_kitchen_type_started_cooking_items + '" data-done-cooking="' + total_kitchen_type_done_items + '" class="single_order fix" style="margin-top:0px;background-color:#b6d6f6;" data-selected="selected" id="order_' + response[key].sale_id + '">';
                    } else {
                        $order_list_left += '<div data-started-cooking="' + total_kitchen_type_started_cooking_items + '" data-done-cooking="' + total_kitchen_type_done_items + '" class="single_order fix" style="margin-top:0px" data-selected="unselected" id="order_' + response[key].sale_id + '">';
                    }
                    ...
                    if (last_key == key) {
                        $order_list_left += '<div data-started-cooking="' + total_kitchen_type_started_cooking_items + '" data-done-cooking="' + total_kitchen_type_done_items + '" class="single_order fix" style="background-color:#b6d6f6;" data-selected="selected" id="order_' + response[key].sale_id + '">';
                    } else {
                        $order_list_left += '<div data-started-cooking="' + total_kitchen_type_started_cooking_items + '" data-done-cooking="' + total_kitchen_type_done_items + '" class="single_order fix" data-selected="unselected" id="order_' + response[key].sale_id + '">';
                    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var key in response) {
                ...
                var table_name = "";
                if (response[key].table) {

                    table_name = (response[key].table.name != null) ? response[key].table.name : "";
                }
                //alert(table_name);
                var waiter_name = (response[key].waiter.manager_name != null) ? response[key].waiter.manager_name : "";
                var customer_name = (response[key].customer.name != null) ? response[key].customer.name : "";
        ...
        error: function () {
            console.log('Left Order Refresh Ajax Error shuvo');
            alert(a_error);
        }
    });
}
...
//working
function print_invoice_and_close(sale_id, payment_method_type, invoice_create_type, paid_amount, due_amount, card_no, tips) {



    if (invoice_create_type == 1) { //if type is 1 then update order status to invoiced order, and update payment method type
        update_order_status_to_invoiced(sale_id, payment_method_type, paid_amount, due_amount, card_no, tips);
    } else if (invoice_create_type == 2) { //then change order status to close, close time update, payment method type update,
        close_order(sale_id, payment_method_type, paid_amount, due_amount, card_no, tips);
    }
    print_invoice(sale_id);
}

//for online order modal
function get_all_online_orders() {
    $.ajax({
        url: base_url + "restaurant/sale/get_all_online_order_ajax",
        method: "GET",
        success: function (response) {
            var orders = JSON.parse(response);
            console.log(orders);
            ...
            for (var key in orders) {
                var customer_name = '';
                var table_name = '';
                if (orders[key].customer) {
                    customer_name = (orders[key].customer.name == null || orders[key].customer.name == "") ? "&nbsp;" : orders[key].customer.name;
                }
                if (orders[key].table) {
                    table_name = (orders[key].table.name != null) ? orders[key].table.name : "&nbsp;";
                }
                online_orders += '<div class="single_online_order fix" id="online_order_' + orders[key].id + '" data-selected="unselected">';
                online_orders += '<div class="first_column column fix">' + orders[key].online_order_no + '</div>';
                online_orders += '<div class="second_column column fix">' + customer_name + '</div>';
                online_orders += '<div class="third_column column fix">' + table_name + '</div>';
                online_orders += '</div>';
            }
            $(".online_order_list_holder .detail_holder ").empty();
            $(".online_order_list_holder .detail_holder ").prepend(online_orders);
        },
        error: function () {
            alert(a_error);
        }
...
function get_all_hold_sales() {
    $.ajax({
        url: base_url + "restaurant/sale/get_all_holds_ajax",
        method: "GET",
        success: function (response) {
            var orders = JSON.parse(response);
            // console.log(orders);
...
function update_order_status_to_invoiced(sale_id, payment_method_type, paid_amount, due_amount, card_no, tips) {


    $.ajax({
        url: base_url + "restaurant/sale/update_order_status_ajax",
        method: "POST",
        data: {
            sale_id: sale_id,
            close_order: false,
...
function close_order(sale_id, payment_method_type, paid_amount, due_amount, card_no, tips) {
...
function get_details_of_a_particular_order_for_modal(sale_id) {
    $.ajax({
        ...
        success: function (response) {
            ...
            var tables_booked = '';
            var order_table = [];
            if (response.order_table != null) {
                order_table = response.order_table;
            }
            ...
            if (order_table.length > 0 ) {
                var w = 1;
                for (var k in order_table) {
                    var single_table = order_table[k];
                    if (w == order_table.length) {
                        tables_booked += single_table.table_name;
                    } else {
                        tables_booked += single_table.table_name + ', ';
                    }
            ...
            if(response.table != null){
                tables_booked = response.table.name;
            }
            else {
                tables_booked = 'None';
            }
            ...
            $('#order_details_waiter_name').html(response.waiter.manager_name);

            $('#order_details_customer_id').html(response.customer_id);
            // $('#order_details_customer_name').html(response.customer_name);
            $('#order_details_customer_name').html(response.customer.name);
            ...
            if (response.order_type == 1) {
                ...
                order_type = 'Take Away';
                order_type_id = 2;
                order_number = 'T ' + response.sale_no;
            } else if (response.order_type == 3) {
                order_type = 'Delivery';
                order_type_id = 3;
                order_number = 'DL ' + response.sale_no;
            }
            ...
            // for (var key in response.items) {
            for (var key in response.sales_details) {

                //construct div
                var this_item = response.sales_details[key];
...
function cancel_order(sale_id) {
    $.ajax({
        url: base_url + "restaurant/sale/cancel_particular_order_ajax",
        method: "POST",
        data: {
            sale_id: sale_id,
        },
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    for (var key in response) {
                        ...
                        //var table_name = (response[key].table.name != null) ? response[key].table.name : "";
                        var waiter_name = (response[key].waiter.manager_name != null) ? response[key].waiter.manager_name : "";
...
function reset_finalize_modal() {
    ...
    $('#tips_amount_input').val('');
    $('#payment_gift_card_no_modal_input').val('');
...
function new_notification_interval() {
    ...
    $.ajax({
        url: base_url + "restaurant/sale/get_new_notifications_ajax",
        method: "POST",
        data: {
        },
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
...
function arrange_info_on_the_cart_to_modify(response) {
    ...
    $("#processing_date_time").html((new Date(response.processing_date_time).toLocaleString("sv-SE") + '').replace(' ','T'));
    $("#hidden_processing_date_time").val((new Date(response.processing_date_time).toLocaleString("sv-SE") + '').replace(' ','T'));
    // console.log((new Date(response.processing_date_time).toLocaleString("sv-SE") + '').replace(' ','T'));
    ...
    $("#total_others_discount").html(response.total_others_discount);
    ...
    if (response.order_type == '1') {
        ...
        //for select floor and floor
        // if (response.table.floor == "" || response.table.floor == 0 || response.table.floor == null) {
        // 	$("#select_floor").val('').trigger("change");
        // } else {
        // 	$("#select_floor").val(2).trigger("change");
        // }

        $("#selected_table").val(response.table_id);
        $("#select_floor").val(response.table.floor_id).trigger("change");


        //console.log([$(this).val(), response.table.floor_id])


        if (response.table.floor_id !== '') {
            $.ajax({
                url: base_url + "restaurant/sale/get_floor_with_tables_ajax",
                method: "GET",
                data:{
                    floor_id: response.table.floor_id
                },
                success: function (data) {
                    var floor = '';
                    var floor_tables = '';
...
function get_customer_for_edit(customer_id) {
    $.ajax({
        url: base_url + "restaurant/sale/get_customer_ajax",
        method: "POST",
        data: {
            customer_id: customer_id,
        },
        ...
        success: function (response) {
            ...
            console.log(response);
            console.log(response.customer.state_id);

            $('#customer_id_modal').val(response.customer.id);
            $('#customer_name_modal').val(response.customer.name);
            $('#customer_phone_modal').val(response.customer.phone);
            $('#customer_email_modal').val(response.customer.email);
            $('#customer_delivery_address_modal').val(response.customer.address);
            $('#customer_note_modal').val(response.customer.note);

            //alert(response.customer.country_id);
            // $("#country").val(response.customer.country_id).change();
            // $("#state").val(response.customer.state_id).change();
            // $("#city").val(response.customer.city_id).change();

            $('#country').val(response.customer.country_id).trigger('change');
            $('#state').val(response.customer.state_id).trigger('change');
            $('#city').val(response.customer.city_id).trigger('change');
            $('#customer_apt_number_modal').val(response.customer.apt);
            $('#customer_zip_modal').val(response.customer.zip);
            $('#customer_code_modal').val(response.customer.code);
            $("#customer_group_modal").val(response.customer.customer_group_id).change();
