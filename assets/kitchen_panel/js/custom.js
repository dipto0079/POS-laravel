...
var kitchen_panel_id = $('base[data-kitchen-panel-id]').attr('data-kitchen-panel-id');
...
$(document).ready(function(){
	...
	$(document).on('click','.start_cooking_button',function(){
		...
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			if($('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length>0){
				...
				// alert(previous_id);
				if(previous_id!=''){
					var url = base_url+"restaurant/kitchen/update_cooking_status_ajax";
					$.ajax({
						url:url,
						method:"POST",
						data:{
			...
			if(previous_id!=''){
				var url = base_url+"restaurant/kitchen/update_cooking_status_delivery_take_away_ajax";
				$.ajax({
					url:url,
					method:"POST",
					data:{
	...
	$('#notification_remove_all').on('click',function(){
		...
		if($('.single_notification_checkbox:checked').length>0){
			...
			if(notifications!=""){
				...
				$.ajax({
					url:base_url+"restaurant/kitchen/remove_multiple_notification_ajax",
					method:"POST",
					data:{
	...
	$(document).on('click','.single_serve_b',function(){
		...
		$.ajax({
			url:base_url+"restaurant/kitchen/remove_notication_ajax",
			method:"POST",
			data:{
...
function new_notification_interval(){
	...
	$.ajax({
		url:base_url+"restaurant/kitchen/get_new_notifications_ajax",
		method:"POST",
		data:{
			kitchen_panel_id: kitchen_panel_id
		},
		headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
		success:function(response) {
...
function refresh_orders() {
	var url = base_url+"restaurant/kitchen/get_new_orders_ajax"; 
	$('#refresh_it_or_not').html('Yes');
	$.ajax({
		url:url,
		method:"POST",
		...
		success:function(response) {
			...
			var i = 1;
			for (var key in response) {
				if (response[key].sales_details != '') {
					// if(i==1){
						// $order_list_left += '<div class="single_order fix" style="margin-top:0px" data-selected="unselected" id="single_order_'+response[key].sale_id+'">';	
					// }else{
					var order_name = '';
					var order_type = '';
					...
					var selected_unselected = ($('#selected_order_for_refreshing_help').html()==response[key].sale_id)?"selected":"unselected";
					var selected_background = ($('#selected_order_for_refreshing_help').html()==response[key].sale_id)?' style="background-color:#b6d6f6" ':'';
					var width = 100;
					...
					var waiter_name = (response[key].waiter_id!=null)?response[key].waiter.manager_name:"";
					// var customer_name = (response[key].customer_id!=null)?response[key].customer.name:"";
					// var booked_time = new Date(Date.parse(response[key].date_time));
					var booked_time = new Date(Date.parse(response[key].processing_date_time));
					var now = new Date();
					...
					// alert([days, hours, minute, second]);
					if(total_kitchen_type_items!=total_kitchen_type_done_items){
						$order_list_left += '<div class="fix floatleft single_order" data-order-type="'+order_type+'" data-selected="'+selected_unselected+'" id="single_order_'+response[key].sale_id+'">';
							$order_list_left += '<div class="header_portion light-blue-background fix">';
								...
								$order_list_left += '<div class="fix floatleft" style="width:30%;">';
									$order_list_left += '<p class="order_duration dark-blue-background"> <span id="kitchen_time_day_'+response[key].sale_id+'">'+days+'d</span><span id="kitchen_time_hour_'+response[key].sale_id+'">'+hours+'h</span><span id="kitchen_time_minute_'+response[key].sale_id+'">'+minute+'</span>:<span id="kitchen_time_second_'+response[key].sale_id+'">'+second+'</span></p>';
								$order_list_left += '</div>';
							$order_list_left += '</div>';
							$order_list_left += '<div class="fix items_holder">';
								
								var items = Object.values(response[key].sales_details);
								console.log(items);
								//for sorting items to ASC
								items = items.sort((a,b) => (a.delay_time > b.delay_time) ? 1 : ((b.delay_time > a.delay_time) ? -1 : 0)); 
								...
								for (var key_item in items) {
									...
									//
									if (single_item.delay_time  == 0 || single_item.delay_time  == null ) {
										var item_background = '';
										var font_style = '';
										...
										$order_list_left += '<div data-selected="unselected" class="fix single_item '+item_background+'" data-order-id="'+response[key].sale_id+'" data-item-id="'+single_item.previous_id+'" id="detail_item_id_'+single_item.previous_id+'" data-cooking-status="'+single_item.cooking_status+'">';
										...
										var processing_delay_time = new Date(response[key].processing_date_time);
										//  processing_delay_time = processing_delay_time.setMinutes(processing_delay_time.getMinutes() + single_item.delay_time);
										processing_delay_time = new Date(processing_delay_time.setMinutes(processing_delay_time.getMinutes() + parseInt(single_item.delay_time)));
										var ct = new Date();

										// console.log([ct, processing_delay_time, single_item.delay_time]);
										if (ct > processing_delay_time) {
											// console.log([ct, processing_delay_time]);
											var item_background = '';
											var font_style = '';
											var cooking_status = 'Not Ready';
											if(single_item.cooking_status=="Done"){
							...
							$order_list_left += '</div>';
							$order_list_left += '<div class="single_order_button_holder" id="single_order_button_holder_'+response[key].sale_id+'">';
								$order_list_left += '<button class="select_all_of_an_order" id="select_all_of_an_order_'+response[key].sale_id+'">Select All</button><button class="unselect_all_of_an_order" id="unselect_all_of_an_order_'+response[key].sale_id+'">Unselect All</button><button class="start_cooking_button" id="start_cooking_button_'+response[key].sale_id+'">Cook</button><button class="done_cooking" id="done_cooking_'+response[key].sale_id+'">Done</button>';    
							$order_list_left += '</div>';
						$order_list_left += '</div>';
					}
					i++;
