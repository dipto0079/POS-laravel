...
$(document).ready(function () {
	...
	//for disable the waiter selection; in waiter is selected based on login credential
	waiterSelect();
	function waiterSelect() {
		if (role == 'Waiter') {
    ...
    $('#create_bill_and_close').on('click', function () {
        ...
        if ($('.holder .order_details > .single_order[data-selected=selected]').length > 0) {
            ...
            let newWindow = open(base_url+"restaurant/sale/print_bill/"+sale_id, 'Print Invoice', 'width=450,height=550')
            newWindow.focus();
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.shift_change == true) {
					...
					$('base[data-shift]').attr('data-shift', data.food_menus[0].shift_id);
					window.items ='';
					...
					for (var i in food_menus ) {
						if (food_menus.hasOwnProperty(key)) {
							var image_path ='';
							if(food_menus [i].photo != null){
								image_path = base_url+'media/restaurant/photos/'+food_menus [i].photo;
							}else{
								image_path = base_url+'assets/images/image_thumb.png';
							}
							category_items_to_show = '<div id="category_'+ food_menus[i].cat_id + '"class="specific_category_items_holder" style="display:flex;">';
							category_items_to_show += '<div class="single_item fix" id="item_' + food_menus[i].id + '">';
							category_items_to_show += '<p class="item_price">$ <span id="price_'+food_menus[i].id+'">'+food_menus[i].dine_in_price+ '</span></p>';
							category_items_to_show += '<img src="' + image_path + '" alt="" width="141">';
							category_items_to_show += '<p class="item_name">' + food_menus [i].name + ' (' + food_menus [i].code + ')</p>';
					...
					for (var key in food_menus) {
						var image_path ='';
						if(food_menus[key].photo != null){
							image_path = base_url+'media/restaurant/photos/'+food_menus[key].photo;
						}else{
							image_path = base_url+'assets/images/image_thumb.png';
						}
						if (food_menus.hasOwnProperty(key)) {
							searched_category_items_to_show += '<div class="single_item fix" id="item_' + food_menus[key].id + '">';
							searched_category_items_to_show += '<p class="item_price">$ <span id="price_'+food_menus[i].id+'">'+food_menus[i].dine_in_price+ '</span></p>';
							searched_category_items_to_show += '<span class="item_vat_percentage" style="display:none;">' + food_menus[key].tax_info + '</span>';
							searched_category_items_to_show += '<img src="' + image_path + '" alt="" width="141">';
							searched_category_items_to_show += '<p class="item_name">' + food_menus[key].name + ' (' + food_menus[key].code + ')</p>';
							searched_category_items_to_show += '</div>';
						}
					}
					searched_category_items_to_show += '<div>';
	...
	//waiter didn't have permission to cancel a order
