                    ...
                    <div class="box box-primary">
                        ...
                        <div class="box-body table-responsive">
                            ...
                            <table id="datatable" class="table table-striped">
                                ...
                                <tbody>
                                @foreach($daly as $v_daly)
                                    <tr>
                                        <td>{{$v_daly->id}}</td>
                                        <td>{{$v_daly->sale_no}}</td>
                                        <td>{{$v_daly->customername}}</td>
                                        <td>{{$v_daly->total_items}}</td>
                                        <td>{{$v_daly->sub_total}}</td>
                                        <td>{{$v_daly->paid_amount}}</td>
                                        <td>{{$v_daly->due_total}}</td>
                                        @if($v_daly->order_status =='1')
                                            <td><a class="btn btn-alart" role="button"
                                                   data-id="{{ $v_daly->order_status }}" data-toggle="modal"
                                                   data-target="#myModal" style="color:#f50808; font-weight: bolder;">New Order</a> </td>
                                        @endif
                                        @if($v_daly->order_status =='2')
                                            <td><a  class="btn btn-alart" role="button"
                                                    data-id="{{ $v_daly->order_status }}" data-toggle="modal"
                                                    data-target="#myModal" style="color:#080ef5;font-weight: bolder;">Invoiced Order</a></td>
                                        @endif
                                        @if($v_daly->order_status =='3')
                                            <td><a  class="btn btn-alart" role="button"
                                                    data-id="{{ $v_daly->order_status }}" data-toggle="modal"
                                                    data-target="#myModal" style="color:#276548; font-weight: bolder;">Closed Order</a></td>
                                        @endif

                                        @if($v_daly->order_type =='1')
                                            <td><a class="btn btn-alart" role="button"
                                                   data-id="{{ $v_daly->order_type }}" data-toggle="modal"
                                                   data-target="#myModal" style="color:#ff0990; font-weight: bolder;">Dine In</a></td>
                                        @endif
                                        @if($v_daly->order_type =='2')
                                            <td><a class="btn btn-alart" role="button"
                                                   data-id="{{ $v_daly->order_type }}" data-toggle="modal"
                                                   data-target="#myModal" style="color:#009cf7; font-weight: bolder;">Take Away</a>
                                            </td>
                                        @endif
                                        @if($v_daly->order_type =='3')
                                            <td><a class="btn btn-alart" role="button"
                                                   data-id="{{$v_daly->order_type }}" data-toggle="modal"
                                                   data-target="#myModal" style="color:#276548; font-weight: bolder;">Delivery</a></td>
                                        @endif
                                        <td>{{$v_daly->order_from}}</td>
                                        <td>{{$v_daly->waiter_id}}</td>
                                        <td>{{$v_daly->usname}}</td>

                                        <td>
                                            <a class="btn btn-primary" role="button" href="Daily-Sale-Report-Details/{{$v_daly->id}}">Details</a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
