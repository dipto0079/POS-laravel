...
// dd($sale_object);
 if($sale_object->order_type=='1'){
    ...
    $order_number = 'T '.$sale_object->sale_no;
 }elseif($sale_object->order_type=='3'){
    $order_type = 'Delivery';
    $order_number = 'DL '.$sale_object->sale_no;    
 }
            ...
            <div id="receiptData">
                ...
                <div id="receipt-data"> 
                    <div style="text-align: center;">
                        ...
                        Date: <?= date('d-m-yy', strtotime($sale_object->sale_date)); ?> <?= date('H:i',strtotime($sale_object->order_time)) ?><br>  
                        Customer: <?= $sale_object->customer->name ?><?= $tables_booked!="" ? " Table No: " . $tables_booked : '' ?><br> 
                        Order Type: <?php echo $order_type; ?><?= $sale_object->order_type=='1' ? " Waiter: " . $sale_object->waiter->name : '' ?> 
                        <br>
                    </div>
                                                ...
                                                if(count($row->modifiers)>0){
                                                    foreach($row->modifiers as $single_modifier){
                                                        if($j==count($row->modifiers)){
                                                            $modifiers_name .= $single_modifier->modifier_name->name;
                                                        }else{
                                                            $modifiers_name .= $single_modifier->modifier_name->name.',';
                                                        }
                                                        $j++;    
