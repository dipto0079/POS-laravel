                        ...
                        <div class="box-body table-responsive">
                            <div class="d-flex justify-content-end" style="padding: 10px 0 30px 0">
                                <a role="button" class="btn btn-primary"
                                   href="{{route('stock-adjustment.create')}}">Add Stock Adjustment
                                </a>
                            </div>
                            <table id="datatable" class="table table-striped">
                                ...
                                @if(count($stockAdjustments) > 0)
                                    @foreach($stockAdjustments as $k=>$v)
                                        <tr>
                                            <th class="text-center" scope="row">{{$k + 1}}</th>
                                            <td class="text-center">{{$v->reference_no}}</td>
                                            <td class="text-center">{{$v->date}}</td>
                                            <td class="text-center">{{count($v->stockAdjustmentIngredients)}}</td>
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <div
                                                        ...
                                                        <a class="dropdown-item edit-link" role="button"
                                                           href="{{route('stock-adjustment.show', [$v->id])}}">Show</a>
                                                        |
                                                        <a class="dropdown-item edit-link" role="button"
                                                           href="{{route('stock-adjustment.edit', [$v->id])}}">Edit</a>
                                                        |
                                                        <a class="dropdown-item delete-stock-adjustment"
