                            ...
                            <div class="box-body">
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label>Ref. No</label>
                                            <input tabindex="1" type="text" id="reference_no" readonly name="reference_no" class="form-control" placeholder="Ref_no" value="{{$waste->reference_no}}">
                                        </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input tabindex="3" type="date" id="date" name="date" class="form-control" value="{{$waste->date}}">
                                        </div>
                                        ...
                                        <div class="form-group"> 
                                            ...
                                            <select tabindex="3" class="form-control select2" id="employee_id" name="employee_id" style="width: 100%;">
                                                ...
                                                @foreach ($employees as $employee)
                                                    <option value="{{$employee->id}}" @if ($employee->id == $waste->employee_id) selected @endif>{{$employee->manager_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>  
                                            ...
                                            <select tabindex="4" class="form-control select2" name="food_menu_id" id="food_menu_id" style="width: 100%;">
                                                ...
                                                foreach ($food_menus as $ingnts) {
                                                    if (!in_array($ingnts->id, $ignoreID)) {
                                                        $ignoreID[] = $ingnts->id;
                                                        ?>
                                                        <option value="{{$ingnts->id}}" @if($waste->food_menu_id == $ingnts->id) selected @endif><?php echo $ingnts->name . " (" . $ingnts->code . ")" ?></option>
                                                    <?php
                                                    }
                                                }
                                                ?>
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input tabindex="1" type="text" id="waste_quantity" name="food_menu_waste_qty" class="form-control" placeholder="waste_quantity" value="{{$waste->food_menu_waste_qty}}">
                                        </div>
                                ...
                                <div class="row">
                                                ...
                                                <tbody>
                                                    ...
                                                    if ($waste->wasteIngredients && !empty($waste->wasteIngredients)) {
                                                        foreach ($waste->wasteIngredients as $wasteIngredient) {
                                                            $i++;
                                                            $unit_price ='';
                                                            $unit_consumption_ingredient = '';
                                                            if ($waste->food_menu_waste_qty) {
                                                                $unit_price = $wasteIngredient->loss_amount/$waste->food_menu_waste_qty;
                                                                $unit_consumption_ingredient = $wasteIngredient->waste_amount/$waste->food_menu_waste_qty;
                                                            }else {
                                                                $unit_price = $wasteIngredient->loss_amount/$wasteIngredient->waste_amount;
                                                                $unit_consumption_ingredient = $wasteIngredient->waste_amount;
                                                            }

                                                            echo '<tr class="rowCount" data-id="' . $i . '" id="row_' . $i . '">' .
                                                            '<td style="padding-left: 10px;"><p id="sl_' . $i . '">' . $i . '</p></td>' .
                                                            '<input type="hidden" id="ingredient_id_' . $i . '" name="ingredient_id[]" value="' . $wasteIngredient->ingredient_id . '"/>' .
                                                            '<td><span style="padding-bottom: 5px;">' . $wasteIngredient->ingredientInfo->name . ' (' . $wasteIngredient->ingredientInfo->code . ')</span></td>' .
                                                            '<td><input type="text" data-countID="' . $i . '" id="waste_amount_' . $i . '" name="waste_amount[]" onfocus="this.select();" class="form-control integerchk aligning" placeholder="Waste Amt" value="' . $wasteIngredient->waste_amount . '" onkeyup="return calculateAll();"/> <span class="label_aligning"> ' . $wasteIngredient->ingredientInfo->unitInfo->name . '</span><span id="unit_consumption_ingredient_'.$i.'" class="unit_consumption_ingredient" style="display:none">'.($unit_consumption_ingredient).'</span></td>' .
                                                            '<input type="hidden" id="last_purchase_price_' . $i . '" name="last_purchase_price[]" value="' . $wasteIngredient->last_purchase_price . '"/>' .
                                                            '<td><input type="text" id="loss_amount_' . $i . '" name="loss_amount[]" class="form-control aligning" placeholder="Loss Amt" value="' . $wasteIngredient->loss_amount . '" readonly /><span class="label_aligning"> $</span><span id="unit_price_ingredient_'.$i.'" class="unit_price_ingredient" style="display:none">'.$unit_price.'</span></td>' .
                                                            '<td><a class="btn btn-danger btn-xs" style="margin-left: 5px; margin-top: 10px;" onclick="return deleter(' . $i . ',' . $wasteIngredient->ingredient_id . ');" ><i class="fa fa-trash"></i> </a></td>' .
                                                            '</tr>';
                                                        }
                                                    }
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <table style="width: 100%">
                                                <tr>
                                                    <td><input style="width: 100%"  class="form-control aligning_total_loss" readonly type="text" name="total_loss" id="total_loss" value="{{$waste->total_loss}}"></td>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label>Note</label>
                                            <textarea tabindex="3" class="form-control" rows="2" id="note" name="note" placeholder="Note ...">{{$waste->note}}</textarea>
                                        </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input required class="form-control integerchk "  id="food_menu_waste_quantity" name="food_menu_waste_quantity" type="text" value="{{$waste->food_menu_waste_qty}}">
                                                </div>
...
@section('script')
    <script>
            ...
            if ($waste->wasteIngredients && !empty($waste->wasteIngredients)) {
                foreach ($waste->wasteIngredients as $wasteIngredient) {
                    $ingredient_id_container .= '"' . $wasteIngredient->ingredient_id . '",';
                }
            }
            ...
            $(function () { 
                ...
                var suffix = <?php if (isset($waste->wasteIngredients)) { echo count($waste->wasteIngredients); } else { echo 0; } ?>;  
                ...
                $('#ingredient_id').on('change',function(){ 
                    ...
                    if (ingredient_details != '') { 
                        ...
                        var cart_row = '<tr class="rowCount" data-id="' + suffix + '" id="row_' + suffix + '">'+
                            ...
                            '<td style="width: 15%"><input required type="text" data-countID="'+suffix+'" id="waste_amount_' + suffix + '" name="waste_amount[]" onfocus="this.select();" class="form-control integerchk aligning" placeholder="waste amount" onkeyup="return calculateAll();"/><span class="label_aligning"> ' + ingredient_details_array[2] + '</span></td>'+
                            '<input type="hidden" id="last_purchase_price_' + suffix + '" name="last_purchase_price[]" value="' + ingredient_details_array[3] + '"/>'+
                            '<td><input type="text"  id="loss_amount_' + suffix + '" name="loss_amount[]" onfocus="this.select();" class="form-control aligning" placeholder="loss amount" readonly /><span class="label_aligning">'+currency+'</span></td>'+ 
